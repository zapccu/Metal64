...
// Generate lookup table to be used in metal code
func generateFloat2() {
    let lookupTable: [Double] = [
        1.0,
        0.5,
        0.25
    ]
...
let arr1: [Double] = Array(repeating: 7.5, count: count)
let arr2: [Double] = Array(repeating: 3.3, count: count)
...
for i in 0..<count {
    ...
    carr3[i] = carr1[i] * carr2[i]
    carr3[i] = carr1[i] / carr2[i]
    carr3[i] = carr1[i] * carr1[i] + carr1[i]
}
print("\nSwift results:")
for i in 0..<3 {
    print("\(carr3[i])")
...
print("\nTime Metal for \(count) elements: \(timeMetal) s = \(1.0/timeMetal) FPS")
print("Time Swift for \(count) elements: \(timeSwift) s =\(1.0/timeSwift) FPS")
